...
<!--
  COMMIT MESSAGE TEMPLATE — 578 EHRM Training App
  -------------------------------------------------------
  HOW TO USE:
...
This commit {{prepares/delivers/fixes}} the **578 EHRM Training App** repository for the **v{{VERSION}}** {{feature/maintenance/patch/security}} release{{— a brief one-line elevator pitch of why this release matters}}. {{1–2 sentences on how the changes were made: e.g., hands-on development in the Power Apps canvas editor, pac export/unpack, documentation-only pass, CI/CD update.}} The updated {{artifact, e.g., `.msapp` package}} (`v{{VERSION}}_578EHRMTrainingApp.msapp`) was {{action, e.g., unpacked via `pac canvas unpack`}} and the resulting source files under `{{src/path/to/files/}}` now reflect the v{{VERSION}} state.
